...
async function changeFiele() {
    //let response = await axios.get(process.env.SYNCURL2);
    let response = await axios.get(process.env.REPOURL+process.env.SYNCURL2);
...
async function start() {
    ...
    if (!process.env.SYNCURL2) {
        console.log("请填写 SYNCURL2 后在继续");
        return;
    }
