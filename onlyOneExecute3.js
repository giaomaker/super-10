...
async function changeFiele() {
    //let response = await axios.get(process.env.SYNCURL3);
    let response = await axios.get(process.env.REPOURL+process.env.SYNCURL3);
...
async function start() {
    ...
    if (!process.env.SYNCURL3) {
        console.log("请填写 SYNCURL3 后在继续");
        return;
    }
